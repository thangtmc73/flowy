...
├── partners/                       # FAQ riêng từng đối tác (flat: {partner_id}_{product_id}.json)
│   ├── msig_health_247.json        # Bảo hiểm Sức khỏe 24/7
│   ├── gic_credit_topup.json       # GIC Credit Topup
│   ├── vbi_cyber.json              # VBI Cyber
│   ├── baoviet_flight_delay_cancel.json
│   └── TEMPLATE.json               # Schema mẫu
...
### Product file - `partners/{partner_id}_{product_id}.json`
...
Tạo file `knowledge/partners/pvi_health_premium.json`:
...
Script sẽ:
- Convert từ `knowledge/faq.json` (old) → `knowledge/partners/msig_health_247.json` (new)
...
- **Product file**: `{partner_id}_{product_id}.json` (ví dụ: `msig_health_247.json`)
...
- Schema questions: xem examples trong `knowledge/partners/msig_health_247.json`
